...
public class DenseMatrixLoopInterchangeMultiplication implements MatrixMultiplication<DenseMatrix, DenseMatrix, DenseMatrix> {
    ...
    public DenseMatrix multiply(DenseMatrix matrix1, DenseMatrix matrix2) {
        ...
        for (int m1Row = 0; m1Row < matrix1.getnRows(); m1Row++) {
            for (int m1Col = 0; m1Col < matrix2.getnColumns(); m1Col++) {
                double sum = 0;
                for (int m2Col = 0; m2Col < matrix1.getnColumns(); m2Col++) {
                    sum += matrix1.get(m1Row, m2Col) * matrix2.get(m2Col, m1Col);
                }
                matrixBuilder.set(m1Row, m1Col, sum);
            }
        }
